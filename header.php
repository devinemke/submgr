		...
		';
			if ($page == 'login' && isset($_SESSION['login']) && $_SESSION['login'])
			{
				...
				if ($_SESSION['contact']['access'] == 'admin' && $submit == 'login')
				{
					...
					if (($version_structure && $version_structure < $version_local) || $missing_configs) {echo '<div class="small notice"><a href="' . $_SERVER['PHP_SELF'] . '?page=' . $page . '&module=maintenance&submodule=update_structure" style="color: red;">Data Structure update required!</a></div>';}
				}
			}
