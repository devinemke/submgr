...
if ($submit == 'login')
{
	...
	$error_output_generic = 'Invalid login. If you have forgotten your password, please visit our <a href="' . $_SERVER['PHP_SELF'] . '?page=help">help page</a>.';
	...
	if ($module == 'maintenance' && $_SESSION['contact']['access'] == 'admin')
	{
		...
		if ($submodule == 'backup')
		{
			...
			if ($submit == 'backup')
			{
				...
				$backup = '';
				$command = $mysqldump_path . ' --host=' . $config_db['host'] . ' --user=' . $config_db['username'] . ' --password=' . $config_db['password'] . ' ' . $config_db['name'];
