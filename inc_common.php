...
function kill_session($arg = '')
{
	...
	if ($arg == 'regenerate') {session_name('submgr'); session_start();}
}
...
function form_main()
{
	...
	<input type="hidden" id="form_hash_main" name="form_hash" value="' . $GLOBALS['nonce'] . '">
	</form>
	';
}
...
function form_confirmation()
{
	...
	<input type="hidden" id="form_hash_confirmation" name="form_hash" value="' . $GLOBALS['nonce'] . '">
	</form>
	';
}
...
function form_login()
{
	...
	if (!$submit) {form_hash('session');} // otherwise $_SESSION['form_hash'] will be overwritten when form_login() and form_main() are together
...
function form_hash($arg)
{
	...
	if ($arg == 'validate')
	{
		if (!isset($_POST['form_hash']) || (isset($_POST['form_hash']) && $_POST['form_hash'] != $_SESSION['form_hash'])) {kill_session('regenerate'); $_SESSION['form_hash'] = $GLOBALS['nonce']; exit_error('form_hash');} else {$_SESSION['form_hash'] = $GLOBALS['nonce'];}
	}
}
